...
class CurrentDocuments:
  ...
  def delete_document(self, document_id):
    ...
    try:
      cur.execute(
          "DELETE from current_documents where id = %s RETURNING *", (document_id,))
      ret = cur.fetchone()
  ...
  def add_document(self, document):
    ...
    try:
      ...
      new_sort_order = last_document['sort_order'] + \
          1 if last_document is not None else 0
  ...
  def reorder_documents(self, username, ordered_document_ids):
    ...
    try:
      ...
      if num_current_documents != len(ordered_document_ids):
        raise Exception(
            "Must pass all document ids for a user to reorder them.")
      for index, document_id in enumerate(ordered_document_ids):
        cur.execute(
            "UPDATE current_documents SET sort_order = %s where id = %s", (index, document_id))
      self._database.commit()
      cur.close()
    except psycopg2.Error:
      self._database.rollback()
