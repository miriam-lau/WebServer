...
class Codenames:
  ...
  def create_game(self, player1, player2) -> int:
    ...
    try:
      ...
      CodenamesDatabase.initialize_locations_for_game(
          cur, game_id, player1, player2, locations)
  ...
  def get_latest_game(self, player) -> Optional[Dict]:
    ...
    try:
      ...
      if game is None:
        return None
      turns_to_hints = CodenamesDatabase.get_turns_to_hints(
          cur, game[CodenamesDatabase.CODENAMES_GAMES_ID])
      turns_to_guesses = CodenamesDatabase.get_turns_to_guesses(
          cur, game[CodenamesDatabase.CODENAMES_GAMES_ID])
      words_for_game = CodenamesDatabase.get_words_for_game(
          cur, game[CodenamesDatabase.CODENAMES_GAMES_ID])
  ...
  def guess(self, game_id, player, guessed_word):
    ...
    try:
      ...
      word_in_game = CodenamesDatabase.get_word_in_game(
          cur, game_id, guessed_word)
      ...
      location_for_word = CodenamesDatabase.get_location_at_index(
          cur, game_id, other_player, word_index)
      if location_for_word is None:
        raise Exception(
            "No location could be found corresponding to that word index")
      ...
      if (location_for_word[CodenamesDatabase.CODENAMES_GAMES_TO_LOCATIONS_LOCATION_TYPE] ==
        ...
        num_found_agents_dict = CodenamesDatabase.get_num_found_agents_for_game(
            cur, game_id)
      ...
      CodenamesDatabase.update_word_status(
          cur, game_id, word_index, new_word_status)
      self._codenames_database.commit()
      cur.close()
    except psycopg2.Error:
      self._codenames_database.rollback()
  ...
  def _generate_locations() -> List[Location]:
    ...
    for i in range(1):
      locations.append(
          Location(Location.LOCATION_TYPE_ASSASSIN, Location.LOCATION_TYPE_AGENT))
    for i in range(5):
      locations.append(
          Location(Location.LOCATION_TYPE_BYSTANDER, Location.LOCATION_TYPE_AGENT))
    for i in range(3):
      locations.append(Location(Location.LOCATION_TYPE_AGENT,
                                Location.LOCATION_TYPE_AGENT))
    for i in range(5):
      locations.append(Location(Location.LOCATION_TYPE_AGENT,
                                Location.LOCATION_TYPE_BYSTANDER))
    ...
    for i in range(1):
      locations.append(
          Location(Location.LOCATION_TYPE_BYSTANDER, Location.LOCATION_TYPE_ASSASSIN))
    for i in range(1):
      locations.append(Location(Location.LOCATION_TYPE_ASSASSIN,
                                Location.LOCATION_TYPE_ASSASSIN))
    for i in range(7):
      locations.append(Location(Location.LOCATION_TYPE_BYSTANDER,
                                Location.LOCATION_TYPE_BYSTANDER))
    for i in range(1):
      locations.append(Location(Location.LOCATION_TYPE_ASSASSIN,
                                Location.LOCATION_TYPE_BYSTANDER))
