...
class RecipesPage:
  ...
  def add_cookbook(self, name, notes):
    ...
    try:
      cur.execute(
          "INSERT INTO cookbooks(name, notes) VALUES(%s, %s) RETURNING *", (name, notes))
      ret = cur.fetchone()
  ...
  def edit_cookbook(self, id, name, notes):
    ...
    try:
      cur.execute(
          "UPDATE cookbooks SET name = %s, notes = %s where id = %s RETURNING *", (name, notes, id))
      ret = cur.fetchone()
      self._database.commit()
      cur.close()
      return ret
  ...
  def get_recipes_page_data(self):
    ...
    try:
      ...
      for cookbook in cur.fetchall():
        ...
        cookbooks_dict[cookbook["parent_id"]
                       ]["children"].append(cookbook["id"])
      ...
      for recipe_meal in cur.fetchall():
        ...
        recipe_dict[recipe_meal["parent_id"]
                    ]["children"].append(recipe_meal["id"])
